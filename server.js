...
// Root route redirects to login.html (Matches vercel.json)
app.get('/', (req, res) => {
...
// Using /tmp for SQLite on Vercel as it's the only writable directory
const dbPath = process.env.VERCEL ? '/tmp/banking_app.db' : path.join(__dirname, 'banking_app.db');
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        db.run(`INSERT INTO users (fullname, email, password) VALUES (?, ?, ?)`, [fullname, email, hashedPassword], function (err) {
            ...
            res.json({ message: 'Success', fullname });
        });
    } catch (e) {
